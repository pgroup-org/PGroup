// Dialog Management
const navMenuParent = document.querySelector(".container header .nav-menu");
const navMenu = document.querySelectorAll(".container header .nav-menu li a");
const teamsMenuItems = document.querySelectorAll(".container main aside .teams-list li a:not(#aside-add-team)");
// const menuItems = document.querySelectorAll(".container main aside .aside .menu .side-menu li a");
const iframe = document.getElementById("user-container-frame");
const userProfileBtn = document.getElementById("userphoto");
const userDialog = document.getElementById("user-dialog");

//Sidemenu Management
const newTeamDialog = document.querySelector(".container main .dialog-bg");
const newTeamButton = document.getElementById("aside-add-team");
const newTeamCreateButton = newTeamDialog.querySelector("#create");
const cancelButton = newTeamDialog.querySelector("#cancel");
var teamsLen =  document.querySelectorAll(".container main aside .teams-list li").length - 1;

navMenu.forEach((item) =>{
  item.addEventListener("click", (e) => {
    iframe.setAttribute("src", `./user/${item.id}.html`);
...
teamsMenuItems.forEach((item) =>{
  item.addEventListener("click", (e) => {
    iframe.setAttribute("src", `./team/${item.id}.html`);
    navMenuParent.querySelector(".active").classList.remove("active");
    navMenuParent.querySelector("#teams").classList.add("active");
    // item.parentElement.parentElement.querySelector(".active").classList.remove("active");
    // item.classList.add("active");
  });
});
...
userProfileBtn.addEventListener("click", (e)=> {
  ...
  var top = getComputedStyle(userDialog).top;
  
  if(top === "-255px"){
    userDialog.style.top = "40px";
  } else if (top === "40px"){
    userDialog.style.top = "-255px";
  }
});
...
newTeamCreateButton.addEventListener("click", (e) =>{
  ...
  if(val1.length !== 0 && val2.length !== 0){
    ...
    newTeamButton.insertAdjacentHTML("beforebegin", `
      <li>
        <a href="#" id="${thisID}">
    ...
    document.querySelector(`.container main aside .teams-list li a#${thisID}`).addEventListener("click", (e) => {
      iframe.setAttribute("src", `./team/${thisID}.html`);
      navMenuParent.querySelector(".active").classList.remove("active");
      navMenuParent.querySelector("#teams").classList.add("active");
    });
